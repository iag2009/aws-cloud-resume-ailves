...
data "aws_iam_policy_document" "repository" {
  ...
  dynamic "statement" {
    ...
    content {
      ...
      principals {  // я изменил список, чтобы можно было добавить arn не из module ecr, а через переменную repository_read_write_access_arns
      // хотя возможно надо оставить права только terraform на запись. Других пользователей можно добавить через консоль
        type = "AWS"
        identifiers = coalescelist(
          var.repository_read_write_access_arns,
          ["arn:${data.aws_partition.current.partition}:iam::${data.aws_caller_identity.current.account_id}:root"],
        )
      }
      actions = [
